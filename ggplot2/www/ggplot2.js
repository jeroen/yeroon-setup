...
Ext.onReady(function(){
	...
	function drawPlot(){
		...
		if(plotConfig.x==null || plotConfig.y==null)
			...
			function updateWorkspace(robjects,files){
				Ext.getCmp('workspace').update('<img src="' + files["plot.png"].value + '" alt="plot"/>');
			...
			RExecuteScript({
				mask: Ext.getCmp('workspace').body,
				scriptname:'gg-plotpng',
				inputs: {'plotConfig': plotConfig},
				robjects: ["plotConfig"],
				files: ["plot.png"],
				success: updateWorkspace,
				failure: failureFn
			});
		}
	}
	...
	function getPDF(){
		...
		if(plotConfig.x==null || plotConfig.y==null)
			...
			function updateWorkspace(robjects,files){
				myWindow.location.href = files["plot.pdf"].value;
				Ext.getCmp('drawButton').enable();
				Ext.getCmp('pdfbutton').enable();
				Ext.getCmp('svgbutton').enable();				
			}
	...
	function getSVG(){
		...
		if(plotConfig.x==null || plotConfig.y==null)
			...
			function updateWorkspace(robjects,files){
				myWindow.location.href = files["plot.svg"].value;
				Ext.getCmp('drawButton').enable();
				Ext.getCmp('pdfbutton').enable();
				Ext.getCmp('svgbutton').enable();				
			}
	...
	function fillData(robjects,objects){
	
		var jsObject = deployr2js(robjects.outputs);
		
		var variableNames = jsObject.variableNames;
		var variableData = jsObject.variableData;
		
		if(typeof(variableNames)=="string"){
			//if there is only one variable, R might return a string instead of array
			variableNames = new Array(variableNames);
		}		
		
		reconfigureGrid(variableNames,variableData);

		Ext.getCmp('guessType').setValue(jsObject.guess.type);
		Ext.getCmp('guessHeader').setValue(jsObject.guess.header);
		Ext.getCmp('guessSep').setValue(jsObject.guess.sep);
		Ext.getCmp('guessDec').setValue(jsObject.guess.dec);		
		
		dataType = jsObject.guess.type;
		...
		if(robjects.outputs.value.error) {
			Ext.getCmp('importButton').disable();
			alert(robjects.outputs.value.error);
		} else {
	...
	function guessData(url){
		...
		RExecuteScript({
			scriptname: 'gg-guessdata',
			mask: Ext.getCmp('uploadWindow').getLayoutTarget(),
			inputs: {'filepath':url, 'filename':Ext.getCmp('fileuploadfield').getValue()},
			robjects: ['outputs'],
			success: fillData,
			failure: disableImport
		});	
	}
	
	function uploadData(filebox,value) {	
	
		formID = Ext.getCmp('uploadForm').getForm().id;
	
		Ext.getCmp('importButton').disable();
		RUploadFile({
			mask: Ext.getCmp('uploadWindow').getLayoutTarget(),
			form:formID,
			fileID: 'fileuploadfield',
			success:guessData
		});
	}
	...
	function customImport(){
		...
		function updateGrid(robjects,files){
			jsObject = deployr2js(robjects.outputs);
		...
		Ext.getCmp('importButton').disable();
		RExecuteScript({
			scriptname: 'gg-customimport',
			mask: Ext.getCmp('uploadWindow').getLayoutTarget(),
			inputs: {dataType:dataType,dataHeader:dataHeader,dataSep:dataSep,dataDec:dataDec},
			robjects: ["outputs"],
			success: updateGrid
		});
	}
	...
	function importData(){
		...
		function successCB(robjects,objects){
			myMask.hide();
			Ext.getCmp('importButton').enable();
			var varListing = deployr2js(robjects.outputTree);
			
			function obj2array(obj){
				var returnArray = [];
				for(z in obj) returnArray.push(obj[z]);
				return returnArray;
			}
			
			varListing.factors.children = obj2array(varListing.factors.children);
			varListing.numerics.children = obj2array(varListing.numerics.children);
			varListing.dates.children = obj2array(varListing.dates.children);
			
			var variables = varListing.factors.children.concat(varListing.numerics.children);			
			varListing = obj2array(varListing);
			...
			Ext.getCmp('tree-panel').getRootNode().appendChild(varListing);
			updateGeneralDetails(Ext.getCmp('workspace').plotConfig);	
	...
	function reconfigureGrid(variableNames,variableData){
	
		var columns = new Array();
		for(var i = 0; i < variableNames.length; i++){
			columns[i] = {header: variableNames[i]}
		}
	...
	function convertVariable(dropEvent){
		...
		function updateTree(robjects,files){
			
			inputList = deployr2js(robjects.inputs);
		
			theNode = Ext.getCmp('tree-panel').getNodeById(inputList.varName).remove(false);
			...
			var newname = 'ggfolder_' + inputList.targetType;
			//
			Ext.getCmp('tree-panel').getNodeById(newname).appendChild(newNode);
			var syntaxTargetType = inputList.targetType == "Date" ? inputList.targetType : inputList.targetType.toLowerCase();
			if(inputList.targetType == "Date"){
				var syntax = "myData$" + inputList.varName + " <- as.Date(myData$" + dropEvent.dropNode.id + ", format=\"" + inputList.dateFormat +"\");";
			} else {
				var syntax = "myData$" + inputList.varName + " <- as." + syntaxTargetType +"(" + "myData$" + dropEvent.dropNode.id + ");";
			}
